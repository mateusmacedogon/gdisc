# Direct Android SDK Setup & APK Compilation
...
if (!(Test-Path "$platformsDir\android.jar")) {
    Write-Host "Passo 1: Baixando Android Platform 34 (63MB)..." -ForegroundColor Cyan
    $platformZip = "$env:TEMP\platform-34.zip"
    curl.exe -L -o $platformZip "https://dl.google.com/android/repository/platform-34-ext12_r01.zip"
    ...
    if (Test-Path $platformsDir) { Remove-Item -Recurse -Force $platformsDir }
    Move-Item -Path "$tempP\android-34" -Destination $platformsDir -Force
    Remove-Item -Force $platformZip
    if (Test-Path $tempP) { Remove-Item -Recurse -Force $tempP }
    Write-Host "Platform 34 instalado com sucesso!" -ForegroundColor Green
}

# 4. Build Tools 34.0.0
if (!(Test-Path "$buildToolsDir\aapt2.exe")) {
    Write-Host "Passo 2: Baixando Android Build-Tools 34.0.0 (58MB)..." -ForegroundColor Cyan
    ...
    if (Test-Path $tempBT) { Remove-Item -Recurse -Force $tempBT }
    Write-Host "Build-Tools 34.0.0 instalado com sucesso!" -ForegroundColor Green
}

# 5. Build APK with Gradle
Write-Host "Passo 3: Compilando o APK do GDisC com Gradle..." -ForegroundColor Cyan
Set-Location "c:\Users\mateu\Downloads\gdisc\gdisc\apps\mobile\android"
...
if (Test-Path $apkSource) {
    ...
    Write-Host "SUCESSO! O seu APK esta pronto em: $distDir\GDisC-1.0.0-debug.apk" -ForegroundColor Green
} else {
